...
public class OverviewView extends Panel {
    ...
    private Component complexEventComponent = null;
    private Grid ComplexEventGrid = null;
	...
	public Component getDrawingComponent() {
		if(complexEventComponent == null) {
			complexEventComponent = createContentWrapper(getDrawingGrid());
		}
		return complexEventComponent;
	}
	
	public Grid getDrawingGrid(){
		if(ComplexEventGrid == null) {
			ComplexEventGrid = new Grid("Drawing Quickselect");
			ComplexEventGrid.setSizeFull();
			ComplexEventGrid.addStyleName(ValoTheme.TABLE_NO_HORIZONTAL_LINES);
			
			ComplexEventGrid.setContainerDataSource(new BeanItemContainer<>(ComplexEvent.class));
			ComplexEventGrid.removeAllColumns();
			ComplexEventGrid.addColumn("name").setExpandRatio(4).setHeaderCaption("Name");
			ComplexEventGrid.addColumn("sink").setExpandRatio(1).setHeaderCaption("Sink").setConverter(new Converter<String, Sink>(){

				@Override
				public Sink convertToModel(String value, Class<? extends Sink> targetType, Locale locale)
						throws com.vaadin.data.util.converter.Converter.ConversionException {
					return null;
				}

				@Override
				public String convertToPresentation(Sink value, Class<? extends String> targetType, Locale locale)
						throws com.vaadin.data.util.converter.Converter.ConversionException {
					return value.getName();
				}

				@Override
				public Class<Sink> getModelType() {
					return Sink.class;
				}
			...
			ComplexEventGrid.addColumn("icon").setHeaderCaption("").setWidth(62).setRenderer(new ImageRenderer());
			
			ComplexEventGrid.setSelectionMode(Grid.SelectionMode.SINGLE);
		}
		return ComplexEventGrid;
	}
